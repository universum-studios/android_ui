...
public final class TintManager {
	...
	private static int[] obtainThemeColors(Context context) {
		...
		final TypedArray typedArray = context.obtainStyledAttributes(null, R.styleable.Ui_Theme_Colors);
		...
		if (typedArray != null) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (index == R.styleable.Ui_Theme_Colors_colorControlNormal) {
					colorNormal = typedArray.getColor(index, colorNormal);
				} else if (index == R.styleable.Ui_Theme_Colors_android_disabledAlpha) {
					colorDisabled = Colors.withAlpha(colorNormal, typedArray.getFloat(index, ALPHA_RATIO_DISABLED));
				} else if (index == R.styleable.Ui_Theme_Colors_uiColorErrorHighlight) {
