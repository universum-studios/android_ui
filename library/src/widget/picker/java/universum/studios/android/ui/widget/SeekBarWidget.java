...
public class SeekBarWidget extends SeekBar implements Widget, FontWidget {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		 ...
		 * Process attributes.
		 */
		final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.Ui_SeekBar, defStyleAttr, defStyleRes);
		if (typedArray != null) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (index == R.styleable.Ui_SeekBar_android_enabled) {
					...
					setDiscreteIndicatorTextGravity(typedArray.getInteger(index, DISCRETE_INDICATOR_TEXT_INFO.gravity));
	...
	private final class Decorator extends FontDecorator<SeekBarWidget> {
		...
		void onProcessTintValues(Context context, TypedArray tintArray, int tintColor) {
			...
			if (tintArray.hasValue(R.styleable.Ui_SeekBar_uiDiscreteIndicatorTint)) {
				tintInfo.discreteIndicatorTintList = tintArray.getColorStateList(R.styleable.Ui_SeekBar_uiDiscreteIndicatorTint);
			}
			...
			if (UiConfig.MATERIALIZED) {
				...
				if (tintArray.hasValue(R.styleable.Ui_SeekBar_uiProgressBackgroundTint)) {
					setProgressBackgroundTintList(tintArray.getColorStateList(R.styleable.Ui_SeekBar_uiProgressBackgroundTint));
				}
				...
				if (tintArray.hasValue(R.styleable.Ui_SeekBar_uiThumbTintMode)) {
					setThumbTintMode(TintManager.parseTintMode(
							tintArray.getInt(R.styleable.Ui_SeekBar_uiThumbTintMode, 0),
							PorterDuff.Mode.SRC_IN
					));
				}
				...
				if (tintArray.hasValue(R.styleable.Ui_SeekBar_uiProgressTint)) {
					tintInfo.primaryProgressTintList = tintArray.getColorStateList(R.styleable.Ui_SeekBar_uiProgressTint);
				}
				tintInfo.progressBackgroundTintList = TintManager.createSeekBarProgressBackgroundTintColors(getContext(), tintColor);
				if (tintArray.hasValue(R.styleable.Ui_SeekBar_uiProgressBackgroundTint)) {
					tintInfo.progressBackgroundTintList = tintArray.getColorStateList(R.styleable.Ui_SeekBar_uiProgressBackgroundTint);
				}
				if (tintArray.hasValue(R.styleable.Ui_SeekBar_uiBackgroundTint)) {
					tintInfo.backgroundTintList = tintArray.getColorStateList(R.styleable.Ui_SeekBar_uiBackgroundTint);
				}
