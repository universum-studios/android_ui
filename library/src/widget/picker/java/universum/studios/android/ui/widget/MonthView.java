...
public class MonthView extends ViewWidget implements FontWidget {
	...
	/**
	 * Creates a new instance of MonthView within the given <var>context</var>.
	 *
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		 ...
		 * Process attributes.
		 */
		final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.Ui_MonthView, defStyleAttr, defStyleRes);
		if (typedArray != null) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (index == R.styleable.Ui_MonthView_android_horizontalSpacing) {
					this.mSpacingHorizontal = typedArray.getDimensionPixelSize(index, mSpacingHorizontal);
				} else if (index == R.styleable.Ui_MonthView_android_verticalSpacing) {
					this.mSpacingVertical = typedArray.getDimensionPixelSize(index, mSpacingVertical);
				} else if (index == R.styleable.Ui_MonthView_uiMonthDayLettersOffsetVertical) {
					this.mDayLettersOffsetVertical = typedArray.getDimensionPixelSize(index, mDayLettersOffsetVertical);
				} else if (index == R.styleable.Ui_MonthView_uiMonthDayNumbersOffsetVertical) {
					this.mDayNumbersOffsetVertical = typedArray.getDimensionPixelOffset(index, mDayNumbersOffsetVertical);
				} else if (index == R.styleable.Ui_MonthView_uiMonthTitleTextAppearance) {
					setTitleTextAppearance(typedArray.getResourceId(index, 0));
				} else if (index == R.styleable.Ui_MonthView_uiMonthDayLetterTextAppearance) {
					setDayLetterTextAppearance(typedArray.getResourceId(index, 0));
				} else if (index == R.styleable.Ui_MonthView_uiMonthDayNumberTextAppearance) {
					setDayNumberTextAppearance(typedArray.getResourceId(index, 0));
				} else if (index == R.styleable.Ui_MonthView_uiMonthDaySelector) {
					setDaySelector(typedArray.getResourceId(index, 0));
				} else if (index == R.styleable.Ui_MonthView_uiMonthDaySelectorRadius) {
					setDaySelectorRadius(typedArray.getDimensionPixelSize(index, mDaySelectorRadius));
				} else if (index == R.styleable.Ui_MonthView_uiMonthCurrentDayTextColor) {
					setCurrentDayTextColor(typedArray.getColorStateList(index));
				}
			}
			typedArray.recycle();
