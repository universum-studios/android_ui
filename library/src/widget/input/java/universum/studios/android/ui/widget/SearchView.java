...
public class SearchView extends LinearLayoutWidget {
	 ...
	 */
	private final OnClickListener CLICK_LISTENER = new OnClickListener() {
	...
	/**
	 * Creates a new instance of SearchView within the given <var>context</var>.
	 *
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		 ...
		 * Process attributes.
		 */
		final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.Ui_SearchView, defStyleAttr, defStyleRes);
		if (typedArray != null) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (index == R.styleable.Ui_SearchView_android_queryHint) {
					...
					tintMode = TintManager.parseTintMode(
							typedArray.getInt(R.styleable.Ui_SearchView_uiTintMode, 0),
