 ...
 * own PullController that can be accessed via {@link #getPullController()}. As this widget can have
 * different orientations depending on its layout manager, it can be pulled along both axis but not
 * at once.
 *
 * <h3>Sliding</h3>
 ...
 * @see GridViewWidget
 */
public class RecyclerViewWidget extends RecyclerView implements Widget, Pullable, Refreshable {
	...
	public Parcelable onSaveInstanceState() {
		...
		if (adapter != null) {
			...
			if (adapterState != null) {
				final AdapterViewWidget.SavedState savedState = new AdapterViewWidget.SavedState(super.onSaveInstanceState());
	...
	public void onRestoreInstanceState(Parcelable state) {
		if (!(state instanceof AdapterViewWidget.SavedState)) {
			super.onRestoreInstanceState(state);
			return;
		}
		final AdapterViewWidget.SavedState savedState = (AdapterViewWidget.SavedState) state;
	...
	private final class Decorator extends RefreshableDecorator<RecyclerViewWidget> {
		...
		void onProcessTypedValues(Context context, TypedArray typedArray) {
			...
			for (int i = 0; i < n; i++) {
				...
				if (index == R.styleable.Ui_RecyclerView_uiPullEnabled) {
					setPullEnabled(typedArray.getBoolean(index, false));
				} else if (index == R.styleable.Ui_RecyclerView_uiRefreshEnabled) {
