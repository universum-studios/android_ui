...
public class FloatingActionButton extends ImageButtonWidget {
	...
	/**
	 * Creates a new instance of FloatingActionButton within the given <var>context</var>.
	 *
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		 ...
		 * Process attributes.
		 */
		final TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.Ui_FloatingActionButton, defStyleAttr, defStyleRes);
		if (typedArray != null) {
			final int n = typedArray.getIndexCount();
			for (int i = 0; i < n; i++) {
				final int index = typedArray.getIndex(i);
				if (index == R.styleable.Ui_FloatingActionButton_uiFabSize) {
					setSize(typedArray.getInteger(index, mSizeType));
				}
			}
			typedArray.recycle();
		}
